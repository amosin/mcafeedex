...
console.log(account);
var socket = io.connect("https://socket.switchdex.ag:8443", {
...
$("#bodyContainer").bind("mousewheel DOMMouseScroll", function (e) {
  ...
  var loc =
    $("#balanceTableDIV").scrollTop() + $("#balanceTableDIV").innerHeight();
  var height = $("#balanceTableDIV")[0].scrollHeight - 10;
  var height2 =
    $("#balanceTableDIV")[0].scrollHeight - $("#balanceTableDIV").innerHeight();
  if (delta < 0) {
    //scroll down
...
function HandleBalances(balances) {
  for (var i = 0; i < balances.length; i++) {
    var td = document.getElementById(
      "sdexbalance" + balances[i].address.toLowerCase()
    );
...
function HandleWalletBalance(data) {
  if (data.balance) {
    ...
    var td = document.getElementById(
      "walletbalance" + data.address.toLowerCase()
    );
...
function TokenMatches(text) {
  var as = $(tokens).filter(function (i, n) {
    return (
      n.name.toLowerCase().includes(text.toLowerCase()) ||
      n.addr.toLowerCase().includes(text.toLowerCase()) ||
      n.name.toUpperCase().includes(text.toUpperCase()) ||
      n.addr.toUpperCase().includes(text.toUpperCase())
    );
  });
  return as;
}
...
function GetEthPrice(tokens) {
  ...
  var settings = {
    ...
    headers: {
      ...
      "Postman-Token":
        "8eb0102e-0971-4585-b790-1a315e2d3ab6,c2ecc056-1d50-4cef-b1d8-3cd80641f9fe",
      "cache-control": "no-cache",
    },
    processData: false,
...
function CreateRow(token) {
  ...
  td7.innerHTML =
    ...
    token.name +
    '\')" class="walletDepositButton">Open Market</button>';
  tr.appendChild(td7);
